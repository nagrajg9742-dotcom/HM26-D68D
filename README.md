...
|---|---|---|
| `<Mysuru resident>` | `<Reports issues such as overflowing garbage, potholes, blocked drains, or damaged streetlights and may not know what happens after submitting the complaint.>` | `<Report once, track the complaint, see its status and assignment, and know when action is taken.>` |
| `<Mysuru civic officer>` | `<Handles multiple complaints and needs visibility into complaints that are pending, inactive, overdue, or requiring attention.>` | `<View, prioritize, assign, update, and resolve complaints with a clear status history.>` |
| `<Field / sanitation worker>` | `<Receives civic work through assigned complaints and needs clear information about what issue requires action.>` | `<See assigned work, understand the complaint, update progress, and support resolution with evidence where required.>` |
...
**Screenshots:** `<**Screenshots:** Screenshots of the citizen complaint submission, officer dashboard, and complaint details/risk intelligence views will be added under `docs/images/` before final submission. Each image will be kept below 1 MB.
>`
...
➡️ Diagram, components, data model and APIs: docs/architecture.md>'
...
➡️ Full decision log: **[resource.md](./resource.md)** · Template: **[decision-log.md](./resource-templates/decision-log.md)**
